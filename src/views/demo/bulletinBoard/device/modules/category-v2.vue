 ...
 * @LastEditTime: 2023-05-12 15:35:31
 * @Description: 
 * @LastEditors: lyj
-->
<template>
...
<script lang="ts" setup>
  ...
  const eChart: any = ref('eChartCategory' + Date.now() + Math.random()) //生成id 防止切换路由不渲染
  ...
  const init = () => {
    ...
    const myChart = _ref ? echarts.getInstanceByDom(_ref) : undefined
    if (myChart !== undefined) {
      //只有获取到dom后才执行
      myChart.clear() //清空数据重新赋值
      echartsOpt.series[0].data = [state.data]
      echartsOpt.title[1].text = state.sum
      echartsOpt.title[0].text = state.content
      echartsOpt.title[1].y = '40%'
      myChart.setOption(echartsOpt)
    }
  }
