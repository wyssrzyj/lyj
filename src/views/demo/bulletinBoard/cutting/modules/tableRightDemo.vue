      ...
      <div class="table_body">
        <div class="table_main_body" :style="{ height: state.height + 'vh' }">
          <div class="table_inner_body" :style="{ top: state.tableTop + 'px' }">
            <!-- 内容 -->
            <tableRightNo v-for="(item, index) in Data" :key="index" :height="31" :title="item.title" :data="item.data" />
          </div>
        </div>
      </div>
    </div>
...
<script lang="ts" setup>
  ...
  import { exportData, Data } from './conifgsRight'
  import tableRightNo from './tableRightNo.vue'
