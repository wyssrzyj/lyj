        ...
        <div class="table_main_body" :style="{ height: state.height + 'vh' }">
          <div class="table_inner_body">
            <div v-for="(item, index) in state.tableList" :key="index" class="table_tr">
              ...
              <div class="tr6 tr newColorType">
                <img :src="newDataImg.get(item.type)" alt="" class="tableIcon" />
                <div :style="{ color: newColor.get(item.type) }">{{ newData.get(item.type) }}</div>
              </div>
            </div>
          </div>
        </div>
...
<script lang="ts" setup>
  ...
  import { newData, newDataImg, newColor } from './conifgsRight'
